...
export function Subject({ subjectData }) {
  const { getSubjectState, setSubjectState, getSubjectByCode } = usePensum();

  const currentState = getSubjectState(subjectData["codigo"]);
  const styles = STATE_STYLES[currentState];

  const handleClickSubject = () => {
    if (getSubjectByCode(subjectData.codigo)) {
      ...
      >
        <div className='flex justify-between'>
          ...
          <button className="btn-ghost" onClick={(e) => handleClickSubjectOptions(e)}>
            <svg xmlns="http://www.w3.org/2000/svg" height="12" width="10.5" viewBox="0 0 448 512"><path fill="rgb(106, 114, 130)" d="M0 256a56 56 0 1 1 112 0 56 56 0 1 1 -112 0zm168 0a56 56 0 1 1 112 0 56 56 0 1 1 -112 0zm224-56a56 56 0 1 1 0 112 56 56 0 1 1 0-112z" /></svg>
          </button>
        </div>
